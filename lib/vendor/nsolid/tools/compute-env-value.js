...
const VarFns = {
  ...
  NSOLID_CONSOLE_PUBKEY: get_NSOLID_CONSOLE_PUBKEY
}

// get VCAP env vars as JSON value
const VcapApplication = cfEnv.getVcapApplication()

// get credentials for user-provided service for console
...
const Tunnel = ConsoleCredentials.tunnel

const ConsoleApp = (Tunnel === 'cf-ssh')
  ? ConsoleCredentials['consoleApp']
  : null

const Sockets = (Tunnel == null)
  ? ConsoleCredentials['sockets']
  : null

if (ConsoleApp == null && Sockets == null) {
  error(`the user-provided service named ${ConsoleServiceName} is invalid`)
  process.exit(1)
}
...
// Return an appropriate NSOLID_CONSOLE_PUBKEY value
function get_NSOLID_CONSOLE_PUBKEY (envVar, envVal) {
  if (ConsoleCredentials.publicKey == null) return
  return ConsoleCredentials.publicKey
}
